class NPC {
    ...
    constructor(name, hitpoints, is_hero = false) {
        this.name = name ? name : ++NPC.NPCs;
        this.hitpoints = hitpoints ? hitpoints : Math.floor(Math.random() * 100 + 1);
        this.is_hero = is_hero
        this.is_alive = true
    }

    status() {
        return `<Status name=${this.name}, hp=${this.hitpoints}/>`;
    }
}

class Hero extends NPC {
    constructor(name, hitpoints = 100, damage = 10, has_critical = false) {
        super(name, hitpoints, true);
        this.damage = damage;
...
heroes_left.filter(it => it.has_critical).forEach(it => console.log(it.status()))
